...
- Subject: "Order Confirmation", "Thank you for your Urban Outfitters order!",
           "Cancellation Notice", "Your order has shipped"
...
- Products are listed in table rows with class containing "item-table-container"
  (Gmail forwarding may prefix with "m_XXXXX_" — parser uses regex match, not exact)
- Each product has:
  ...
  - Quantity: In <td> with class "item-price-large" and align="center"

UNIQUE ID CROSSOVER:
  Status: Feasible (with slug normalization)
  Email source: Product name (slugified) + color code from image URL
  URL regex: r"/shop/(?:.+/)?([^/?]+)" + r"[?&]color=(\\d{3})"
  Format: {normalized-slug}-{color_code}
    where normalized-slug = product slug with "womens"/"mens" segments
    removed AND trailing digits stripped.

  Examples:
    Email "Nike Women's Cortez Leather Sneaker" + color 010
      -> "nike-cortez-leather-sneaker-010"
    URL /shop/nike-womens-cortez-leather-sneaker?color=010
      -> "nike-cortez-leather-sneaker-010"  (match)

    Email "Gola Women's Elan Sneaker" + color 020
      -> "gola-elan-sneaker-020"
    URL /shop/gola-elan-sneaker2?color=020
      -> "gola-elan-sneaker-020"  (match after trailing-digit strip)

    Email "On Cloudnova 2 Women's Sneaker" + color 055
      -> "on-cloudnova-2-sneaker-055"
    URL /shop/on-womens-cloudnova-2-sneaker?color=055
      -> "on-cloudnova-2-sneaker-055"  (match after "womens" strip)

  Notes:
    - /shop/ path may have optional middle segment (e.g., /shop/hybrid/slug).
      URL regex captures the LAST segment before the query string.
    - Normalization is applied to BOTH sides (email parser + URL extractor)
      to guarantee matching IDs. See lambda-purchase-tracker/src/unique_id_extractor.py
"""
...
class UrbanOutfittersEmailParser:
    ...
    DEV_SUBJECT_ORDER_PATTERN = r"(?:Fwd:\s*)?(?:order\s+confirmation|thank you for your.*order)"
    ...
    def _extract_urban_product_details(self, container) -> Optional[UrbanOrderItem]:
        ...
        try:
            ...
            # Generate unique ID: {normalized-slug}-{color_code}
            # Convert product name to slug, then normalize (strip womens/mens + trailing digits)
            product_slug = self._product_name_to_slug(product_name)
            normalized_slug = self._normalize_slug(product_slug)
            unique_id = f"{normalized_slug}-{color_code}"
            details['unique_id'] = unique_id
            logger.debug(f"Generated unique ID: {unique_id} (raw slug: {product_slug})")
