"""
Test script to parse Shoe Palace order confirmation emails and verify extraction.

Usage:
    python -m test.test_shoepalace                    # Search Gmail for first email
    python -m test.test_shoepalace shoepalace.txt     # Use specific email file
    python test/test_shoepalace.py                    # Run directly (from backend dir)
"""
...
def test_shoepalace_email_from_gmail():
    """Test parsing the first Shoe Palace order confirmation email from Gmail"""
    
    print("Searching Gmail for Shoe Palace order confirmation emails...")
    print("=" * 80)
    ...
    try:
        ...
        # For dev, add shopifyemail to distinguish from other retailers
        if parser.settings.is_development:
            query = f'from:{from_email} subject:"confirmed" shopifyemail'
        else:
        ...
        if not message_ids:
            print("❌ No Shoe Palace order confirmation emails found")
            return None
        ...
        # Validate it's actually a Shoe Palace email before parsing
        if not parser.is_shoepalace_email(email_data):
            print(f"❌ Email is not from Shoe Palace (from: {email_data.sender})")
            return None
        
        if not parser.is_order_confirmation_email(email_data):
            print(f"❌ Email is not a Shoe Palace order confirmation (subject: {email_data.subject})")
            return None
        
        # Parse the email
        print("\nParsing Shoe Palace order confirmation email...")
...
def test_shoepalace_email(email_file_name: str):
    """Test parsing a Shoe Palace order confirmation email file"""
    ...
    print(f"Reading Shoe Palace email from: {email_file}")
    print("=" * 80)
    ...
    email_data = EmailData(
        ...
        sender="store+8523376@t.shopifyemail.com",
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        test_shoepalace_email(email_file_name)
    else:
        # Search Gmail for first email
        test_shoepalace_email_from_gmail()
