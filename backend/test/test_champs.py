"""
Test script to parse Champs Sports order confirmation emails and verify extraction.

Usage:
    python -m test.test_champs                    # Search Gmail for first email
    python -m test.test_champs champs.txt         # Use specific email file
    python test/test_champs.py                    # Run directly (from backend dir)
"""
...
def test_champs_email_from_gmail():
    """Test parsing the first Champs Sports order confirmation email from Gmail"""
    
    print("Searching Gmail for Champs Sports order confirmation emails...")
    print("=" * 80)
    
    try:
        gmail_service = GmailService()
        parser = ChampsEmailParser()
        ...
        if not message_ids:
            print("❌ No Champs Sports order confirmation emails found")
            return None
        ...
        # Parse the email
        print("\nParsing Champs Sports order confirmation email...")
...
def test_champs_email(email_file_name: str):
    """Test parsing a Champs Sports order confirmation email file"""
    ...
    print(f"Reading Champs Sports email from: {email_file}")
    print("=" * 80)
    ...
    email_data = EmailData(
        ...
        sender="accountservices@em.champssports.com",
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        test_champs_email(email_file_name)
    else:
        # Search Gmail for first email
        test_champs_email_from_gmail()
