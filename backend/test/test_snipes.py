"""
Test script to parse Snipes order confirmation emails and verify extraction.

Usage:
    python -m test.test_snipes                    # Search Gmail for first email
    python -m test.test_snipes snipes.txt         # Use specific email file
    python test/test_snipes.py                    # Run directly (from backend dir)
"""
...
def test_snipes_email_from_gmail():
    """Test parsing the first Snipes order confirmation email from Gmail"""
    
    print("Searching Gmail for Snipes order confirmation emails...")
    print("=" * 80)
    
    try:
        gmail_service = GmailService()
        parser = SnipesEmailParser()
        ...
        if not message_ids:
            print("❌ No Snipes order confirmation emails found")
            return None
        ...
        # Parse the email
        print("\nParsing Snipes order confirmation email...")
...
def test_snipes_email(email_file_name: str):
    """Test parsing a Snipes order confirmation email file"""
    ...
    print(f"Reading Snipes email from: {email_file}")
    print("=" * 80)
    
    with open(email_file, 'r', encoding='utf-8') as f:
        html_content = f.read()
    
    # Create EmailData object - use Snipes subject format for order number extraction from subject
    email_data = EmailData(
        message_id="test-123",
        thread_id="test-thread-123",
        subject="Confirmation of Your SNIPES Order #SNP22183213",
        sender="no-reply@snipesusa.com",
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        test_snipes_email(email_file_name)
    else:
        # Search Gmail for first email
        test_snipes_email_from_gmail()
