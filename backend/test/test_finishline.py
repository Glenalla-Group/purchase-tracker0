"""
Test script to parse Finish Line order confirmation emails and verify extraction.

Usage:
    python -m test.test_finishline                    # Search Gmail for first email
    python -m test.test_finishline finishline.txt     # Use specific email file
    python test/test_finishline.py                    # Run directly (from backend dir)
"""
...
def test_finishline_email_from_gmail():
    """Test parsing the first Finish Line order confirmation email from Gmail"""
    
    print("Searching Gmail for Finish Line order confirmation emails...")
    print("=" * 80)
    ...
    try:
        ...
        if not message_ids:
            print("❌ No Finish Line order confirmation emails found")
            return None
        ...
        # Parse the email
        print("\nParsing Finish Line order confirmation email...")
...
def test_finishline_email(email_file_name: str):
    """Test parsing a Finish Line order confirmation email file"""
    ...
    print(f"Reading Finish Line email from: {email_file}")
    print("=" * 80)
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        test_finishline_email(email_file_name)
    else:
        # Search Gmail for first email
        test_finishline_email_from_gmail()
