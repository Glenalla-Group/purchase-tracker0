"""
Test script to parse Footlocker order confirmation emails and verify extraction.

Usage:
    python -m test.test_footlocker                    # Search Gmail for first email
    python -m test.test_footlocker footlocker1.txt    # Use specific email file
    python test/test_footlocker.py                    # Run directly (from backend dir)
"""
...
def test_footlocker_email_from_gmail():
    """Test parsing the first Footlocker order confirmation email from Gmail"""
    
    print("Searching Gmail for Footlocker order confirmation emails...")
    print("=" * 80)
    ...
    try:
        ...
        if not message_ids:
            print("❌ No Footlocker order confirmation emails found")
            return None
...
def test_footlocker_email_from_file(email_file_name: str):
    """Test parsing a Footlocker order confirmation email file"""
    ...
    print(f"Reading Footlocker email from: {email_file}")
    print("=" * 80)
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        test_footlocker_email_from_file(email_file_name)
    else:
        # Search Gmail for first email
        test_footlocker_email_from_gmail()
