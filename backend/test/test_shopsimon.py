"""
Test script to parse Shop Simon order confirmation emails and verify extraction.

Usage:
    python -m test.test_shopsimon                    # Search Gmail for first email
    python -m test.test_shopsimon shopsimon.txt     # Use specific email file
    python test/test_shopsimon.py                    # Run directly (from backend dir)
"""
...
def test_shopsimon_email_from_gmail():
    """Test parsing the first Shop Simon order confirmation email from Gmail"""
    
    print("Searching Gmail for Shop Simon order confirmation emails...")
    print("=" * 80)
    
    try:
        gmail_service = GmailService()
        parser = ShopSimonEmailParser()
        ...
        if not message_ids:
            print("❌ No Shop Simon order confirmation emails found")
            return None
        ...
        # Parse the email
        print("\nParsing Shop Simon order confirmation email...")
...
def test_shopsimon_email(email_file_name: str):
    """Test parsing a Shop Simon order confirmation email file"""
    ...
    print(f"Reading Shop Simon email from: {email_file}")
    print("=" * 80)
    ...
    email_data = EmailData(
        ...
        sender="onlinesupport@shopsimon.com",
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        test_shopsimon_email(email_file_name)
    else:
        # Search Gmail for first email
        test_shopsimon_email_from_gmail()
