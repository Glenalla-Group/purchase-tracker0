"""
Test script to parse ASOS order confirmation emails and verify extraction.

Usage:
    python -m test.test_asos                    # Search Gmail for first email
    python -m test.test_asos asos.txt           # Use specific email file
    python test/test_asos.py                   # Run directly (from backend dir)
"""
...
def test_asos_email_from_gmail():
    """Test parsing the first ASOS order confirmation email from Gmail"""
    
    print("Searching Gmail for ASOS order confirmation emails...")
    print("=" * 80)
    
    try:
        gmail_service = GmailService()
        parser = ASOSEmailParser()
        ...
        if not message_ids:
            print("❌ No ASOS order confirmation emails found")
            return None
        ...
        # Parse the email
        print("\nParsing ASOS order confirmation email...")
...
def test_asos_email(email_file_name: str):
    """Test parsing an ASOS order confirmation email file"""
    ...
    print(f"Reading ASOS email from: {email_file}")
    print("=" * 80)
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        test_asos_email(email_file_name)
    else:
        # Search Gmail for first email
        test_asos_email_from_gmail()
