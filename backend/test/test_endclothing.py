"""
Test script to parse END Clothing order confirmation emails and verify extraction.

Usage:
    python -m test.test_endclothing                    # Search Gmail for first email
    python -m test.test_endclothing endclothing.txt   # Use specific email file
    python test/test_endclothing.py                   # Run directly (from backend dir)
"""
...
def test_endclothing_email_from_gmail():
    """Test parsing the first END Clothing order confirmation email from Gmail"""
    
    print("Searching Gmail for END Clothing order confirmation emails...")
    print("=" * 80)
    
    try:
        gmail_service = GmailService()
        parser = ENDClothingEmailParser()
        ...
        if not message_ids:
            print("❌ No END Clothing order confirmation emails found")
            return None
        ...
        for idx, item in enumerate(order_data.items, 1):
            ...
            print(f"  Converted Size: {item.size}")
...
def test_endclothing_email(email_file_name: str):
    """Test parsing an END Clothing email file"""
    ...
    print(f"Reading END Clothing email from: {email_file}")
    print("=" * 80)
    ...
    email_data = EmailData(
        ...
        sender="info@orders.endclothing.com",
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        test_endclothing_email(email_file_name)
    else:
        # Search Gmail for first email
        test_endclothing_email_from_gmail()
