"""
Test script to parse Dick's Sporting Goods order confirmation emails and verify extraction.

Usage:
    python -m test.test_dicks                    # Search Gmail for first email
    python -m test.test_dicks dicks.txt          # Use specific email file
    python test/test_dicks.py                    # Run directly (from backend dir)
"""
...
def test_dicks_email_from_gmail():
    """Test parsing the first Dick's Sporting Goods order confirmation email from Gmail"""
    
    print("Searching Gmail for Dick's Sporting Goods order confirmation emails...")
    print("=" * 80)
    
    try:
        gmail_service = GmailService()
        parser = DicksEmailParser()
        ...
        if not message_ids:
            print("❌ No Dick's Sporting Goods order confirmation emails found")
            return None
        ...
        # Parse the email
        print("\nParsing Dick's Sporting Goods order confirmation email...")
...
def test_dicks_email(email_file_name: str):
    """Test parsing a Dick's Sporting Goods order confirmation email file"""
    ...
    print(f"Reading Dick's Sporting Goods email from: {email_file}")
    print("=" * 80)
    ...
    email_data = EmailData(
        ...
        sender="dickssportinggoods@order.email.dickssportinggoods.com",
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        test_dicks_email(email_file_name)
    else:
        # Search Gmail for first email
        test_dicks_email_from_gmail()
