"""
Test script to parse Revolve order confirmation emails and verify extraction.

Usage:
    python -m test.test_revolve                    # Search Gmail for first email
    python -m test.test_revolve revolve.txt       # Use specific email file
    python test/test_revolve.py                   # Run directly (from backend dir)
"""
...
def test_revolve_email_from_gmail():
    """Test parsing the first Revolve order confirmation email from Gmail"""
    
    print("Searching Gmail for Revolve order confirmation emails...")
    print("=" * 80)
    
    try:
        gmail_service = GmailService()
        parser = RevolveEmailParser()
        ...
        if not message_ids:
            print("❌ No Revolve order confirmation emails found")
            return None
        ...
        # Parse the email
        print("\nParsing Revolve order confirmation email...")
...
def test_revolve_email(email_file_name: str):
    """Test parsing a Revolve order confirmation email file"""
    ...
    print(f"Reading Revolve email from: {email_file}")
    print("=" * 80)
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        test_revolve_email(email_file_name)
    else:
        # Search Gmail for first email
        test_revolve_email_from_gmail()
