...
- Shipping Subjects:
    - "Your items are packed and ready to ship" (full shipment)
    - "Part of Your Order is Packed and Ready to Ship" (partial shipment,
      may include multiple tracking numbers across different carriers)
...
- Carrier(s): after "Shipping Carrier" text. Multi-package partial shipments
  list multiple carriers in sibling <td> cells in the row below (e.g. USPS + FEDEX).
- Tracking number(s): after "Tracking Number" text. The row below contains
  one <a> tag per package; partial shipments have multiple <a> tags.
...
- Partial shipments render one "Package N of M" block per package, each with
  its own items. Items aggregate cumulatively into shipped_to_pw.
...
class AcademyShippingData(BaseModel):
    ...
    tracking_numbers: List[str] = Field(
        ...
        description="Tracking number(s) — one per package; partial shipments carry multiple",
    )
    carriers: List[str] = Field(
        default_factory=list,
        description="Shipping carrier(s) — one per package; partial shipments may mix (e.g. USPS + FEDEX)",
    )

    @property
    def tracking_number(self) -> str:
        """Comma-joined tracking string for sheets_writer / DB `tracking` field compatibility."""
        return ", ".join(self.tracking_numbers)

    @property
    def carrier(self) -> Optional[str]:
        """Comma-joined carrier string for logging/display compatibility."""
        return ", ".join(self.carriers) if self.carriers else None
...
class AcademyEmailParser:
    ...
    # Matches BOTH "Your items are packed and ready to ship" (full shipment)
    # AND "Part of Your Order is Packed and Ready to Ship" (partial shipment).
    SUBJECT_SHIPPING_PATTERN = (
    ...
    DEV_SUBJECT_SHIPPING_PATTERN = r"(?:Fwd:\s*)?" + SUBJECT_SHIPPING_PATTERN

    # Gmail search queries (plain strings for Gmail API)
    GMAIL_ORDER_QUERY = "Thanks for shopping with us"
    GMAIL_SHIPPING_QUERY = "packed and ready to ship"
    ...
    def parse_shipping_email(self, email_data: EmailData) -> Optional[AcademyShippingData]:
        ...
        try:
            ...
            tracking_numbers = self._extract_tracking_numbers(soup)
            logger.info(f"Academy tracking number(s): {tracking_numbers}")

            carriers = self._extract_carriers(soup)
            logger.info(f"Academy carrier(s): {carriers}")
            ...
            logger.info(f"Academy shipping {order_number}: {len(items)} item(s) shipped across {len(tracking_numbers)} package(s)")
    ...
    def _extract_tracking_numbers(self, soup: BeautifulSoup) -> List[str]:
        """Extract tracking number(s) from shipping email.

        Full shipment emails contain one <a> tag in the row below the
        "Tracking Number" <td>; partial shipments contain multiple
        (one per package). Preserves order.
        """
        ...
        try:
            ...
            logger.error(f"Error extracting Academy tracking numbers: {e}", exc_info=True)

        return tracking_numbers

    def _extract_carriers(self, soup: BeautifulSoup) -> List[str]:
        """Extract shipping carrier(s) from shipping email.

        Full shipment emails list one carrier in the row below the
        "Shipping Carrier" <td>. Partial shipments list each carrier
        inside nested <text> elements separated by <br> tags within a
        single <td> (e.g. "USPS<br>FEDEX<br>"). We split the row's text
        on line breaks to handle both layouts uniformly. Preserves order.
        """
        carriers: List[str] = []
        try:
            for td in soup.find_all('td'):
                ...
                if td_text == "Shipping Carrier":
                    ...
                    if parent:
                        ...
                        if next_row:
                            ...
                            raw = next_row.get_text(separator='\n')
            ...
            if match:
                carriers.append(match.group(1))

        except Exception as e:
            logger.error(f"Error extracting Academy carriers: {e}", exc_info=True)
